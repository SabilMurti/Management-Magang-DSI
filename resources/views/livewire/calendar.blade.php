    ...
    <div class="card p-0 overflow-hidden bg-white border-slate-200 shadow-sm rounded-xl">
        ...
        <div class="p-4 sm:p-6 border-b border-slate-100 flex flex-col md:flex-row justify-between items-center gap-4 bg-slate-50/50">
            ...
            <div class="flex w-full md:w-auto bg-slate-100 p-1 rounded-xl">
                <button wire:click="switchMode('attendance')"
                    class="flex-1 md:flex-none px-4 py-2 rounded-lg text-sm font-semibold transition-all flex items-center justify-center gap-2 {{ $viewMode === 'attendance' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-slate-700' }}">
                    <i class="fas fa-calendar-check"></i> <span class="hidden sm:inline">Kehadiran</span>
                </button>
                <button wire:click="switchMode('tasks')"
                    class="flex-1 md:flex-none px-4 py-2 rounded-lg text-sm font-semibold transition-all flex items-center justify-center gap-2 {{ $viewMode === 'tasks' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-slate-700' }}">
        ...
        <div class="p-4 border-t border-slate-100 bg-slate-50/30 flex flex-wrap gap-4 text-xs">
            @if($viewMode === 'attendance')
                ...
                @endif
            @else
                <div class="flex items-center gap-2"><div class="w-3 h-3 rounded bg-rose-500"></div> Prioritas Tinggi</div>
                <div class="flex items-center gap-2"><div class="w-3 h-3 rounded bg-amber-500"></div> Prioritas Sedang</div>
                <div class="flex items-center gap-2"><div class="w-3 h-3 rounded bg-emerald-500"></div> Prioritas Rendah</div>
            @endif
        </div>
    </div>
