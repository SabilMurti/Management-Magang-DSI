    ...
    <div class="slide-up max-w-6xl mx-auto space-y-6">
            ...
            <div class="flex-1 w-full">
                <div class="flex flex-col sm:flex-row sm:items-center gap-3 mb-2">
                    ...
                    <div class="flex flex-wrap gap-2">
                         <span class="badge badge-{{ $task->priority_color }} uppercase text-[10px] tracking-wider px-2.5 py-1">
                            {{ ucfirst($task->priority) }}
                        </span>
                        <span class="badge badge-{{ $task->status_color }} uppercase text-[10px] tracking-wider px-2.5 py-1">
                            {{ $task->status_label }}
                        </span>
                ...
                <div class="flex items-center gap-2">
                    ...
                    <p class="text-slate-500 text-sm">Diberikan oleh <span class="font-semibold text-slate-600">{{ $task->assignedBy->name }}</span></p>
                </div>
            </div>
                ...
                <div class="card p-6 sm:p-8">
                    ...
                    <div class="relative pl-3">
                        ...
                        <div class="mb-8 relative pl-8">
                            <div class="absolute left-0 w-4 h-4 bg-white border-2 border-indigo-500 rounded-full ring-4 ring-indigo-500/10 z-10"></div>
                        ...
                        @if($task->started_at)
                             <div class="mb-8 relative pl-8">
                                <div class="absolute left-0 w-4 h-4 bg-white border-2 border-cyan-500 rounded-full ring-4 ring-cyan-500/10 z-10"></div>
                                <div class="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-0.5">Mulai Dikerjakan</div>
                                <strong class="text-slate-700 text-sm block">{{ $task->started_at->format('d M Y') }}</strong>
                        ...
                        @if($task->submitted_at)
                             <div class="relative pl-8">
                                <div class="absolute left-0 w-4 h-4 bg-white border-2 {{ $task->is_late ? 'border-amber-500 ring-amber-500/10' : 'border-emerald-500 ring-emerald-500/10' }} rounded-full ring-4 z-10"></div>
                                <div class="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-0.5">Dikumpulkan</div>
                                <strong class="text-slate-700 text-sm block">{{ $task->submitted_at->format('d M Y') }}</strong>
                                <div class="text-xs text-slate-500">{{ $task->submitted_at->format('H:i') }} WIB</div>
                                @if($task->is_late)
                                    <div class="mt-2 inline-flex px-2 py-0.5 rounded text-[10px] font-bold bg-amber-50 text-amber-600 border border-amber-200">TERLAMBAT</div>
                                @else
                                    <div class="mt-2 inline-flex px-2 py-0.5 rounded text-[10px] font-bold bg-emerald-50 text-emerald-600 border border-emerald-200">ON TIME</div>
                                @endif
                            </div>
                        @endif
                    </div>
                         ...
                         <div class="p-6 text-center">
                            <div class="relative w-28 h-28 mx-auto mb-4">
                                ...
                                <svg class="w-full h-full transform -rotate-90 text-slate-100" viewBox="0 0 36 36">
                                    <path d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" fill="none" stroke="currentColor" stroke-width="2.5" />
                                    <path d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" fill="none" stroke="#22c55e" stroke-width="2.5" stroke-dasharray="{{ $task->score }}, 100" stroke-linecap="round" class="drop-shadow-sm" />
                                </svg>
                                <div class="absolute inset-0 flex flex-col items-center justify-center">
                                    <span class="text-3xl font-black text-emerald-700 leading-none">{{ $task->score }}</span>
                                    <span class="text-[9px] font-bold text-emerald-500 uppercase">POIN</span>
                                </div>
                            </div>

                            <div class="text-lg font-bold text-emerald-700 mb-1">
                                @if($task->score >= 90) Excellent Job!
                                @elseif($task->score >= 75) Good Job!
                                @else Keep Going!
                                @endif
                            </div>
                            <div class="text-xs text-slate-500 mb-6">Dinilai pada {{ $task->approved_at?->format('d M Y') }}</div>
        ...
        @if($task->status === 'revision')
            <div class="rounded-2xl border border-amber-200 bg-white overflow-hidden shadow-sm">
                <div class="bg-amber-50/50 p-6 border-b border-amber-100 flex items-start gap-5">
                    <div class="w-12 h-12 rounded-xl bg-amber-100 border border-amber-200 flex items-center justify-center shrink-0">
                    ...
                    <div>
                        ...
                        <p class="text-sm text-amber-700">Tugas Anda memerlukan revisi. Silakan perbaiki berdasarkan catatan di bawah ini.</p>
                    </div>
                </div>
                <div class="p-6">
                    <label class="text-xs font-bold text-slate-400 uppercase tracking-wider block mb-2">Catatan Pembimbing</label>
                    <div class="bg-slate-50 border-l-4 border-amber-500 rounded-r-xl p-5 mb-5 text-slate-700 text-sm leading-relaxed">
                        "{{ $task->admin_feedback ?? 'Mohon perbaiki tugas sesuai dengan instruksi awal.' }}"
                    </div>
                    @if(auth()->user()->isIntern())
                        <div class="flex items-center gap-3 px-4 py-3 bg-rose-50 border border-rose-100 rounded-xl text-rose-700 text-sm font-medium">
                            <i class="fas fa-info-circle"></i>
        ...
        @if(auth()->user()->isIntern() && ($task->status !== 'completed' && $task->status !== 'submitted'))
            <div class="card p-0 overflow-hidden" x-data="linksForm()">
                ...
                @if($task->isOverdue())
                    <div class="mx-6 mt-6 p-4 bg-amber-50 text-amber-700 border border-amber-200 rounded-xl flex items-center gap-3 text-sm font-medium">
                    ...
                    <form action="{{ route('tasks.submit', $task) }}" method="POST" class="space-y-6">
                        ...
                        <div class="form-group mb-0">
                            ...
                            <textarea name="submission_notes" class="form-control" rows="3"
                                placeholder="Tambahkan catatan...">{{ old('submission_notes', $task->submission_notes) }}</textarea>
                        </div>
        ...
        @if(auth()->user()->canManage() && $task->status === 'submitted')
            <div class="card p-0 overflow-hidden border-2 border-indigo-100 shadow-lg">
                ...
                <form action="{{ route('tasks.review', $task) }}" method="POST" class="p-6 md:p-8">
                    ...
                    <div class="mb-8">
                        <label class="text-xs font-bold text-slate-400 uppercase tracking-wider block mb-3">Keputusan Pembimbing</label>
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <!-- Approve Option -->
                            <label class="cursor-pointer group">
                                <input type="radio" name="action" value="approve" checked onchange="toggleScoreInput(true)" class="peer hidden" id="radioApprove">
                                <div class="p-5 rounded-2xl border-2 border-slate-200 bg-white hover:border-emerald-300 peer-checked:border-emerald-500 peer-checked:bg-emerald-50/30 transition-all text-center h-full flex flex-col items-center justify-center gap-3 group-hover:shadow-md">
                                    <div class="w-12 h-12 rounded-full bg-emerald-100 text-emerald-600 flex items-center justify-center text-xl transition-transform group-hover:scale-110">
                                        <i class="fas fa-check"></i>
                                    </div>
                                    <div>
                                        <div class="font-bold text-slate-700 peer-checked:text-emerald-700 text-base">Terima & Nilai</div>
                                        <div class="text-sm text-slate-400 peer-checked:text-emerald-600">Tugas sudah sesuai</div>
                                    </div>
                                </div>
                            </label>
                            ...
                            <!-- Revision Option -->
                            <label class="cursor-pointer group">
                                <input type="radio" name="action" value="revision" onchange="toggleScoreInput(false)" class="peer hidden" id="radioRevision">
                                <div class="p-5 rounded-2xl border-2 border-slate-200 bg-white hover:border-amber-300 peer-checked:border-amber-500 peer-checked:bg-amber-50/30 transition-all text-center h-full flex flex-col items-center justify-center gap-3 group-hover:shadow-md">
                                    <div class="w-12 h-12 rounded-full bg-amber-100 text-amber-600 flex items-center justify-center text-xl transition-transform group-hover:scale-110">
                                        <i class="fas fa-redo"></i>
                                    </div>
                                    <div>
                                        <div class="font-bold text-slate-700 peer-checked:text-amber-700 text-base">Minta Revisi</div>
                                        <div class="text-sm text-slate-400 peer-checked:text-amber-600">Kembalikan ke siswa</div>
                                    </div>
                                </div>
                            </label>
                        </div>
                    ...
                    <div id="scoreInputGroup" class="mb-6 p-5 bg-slate-50 border border-slate-200 rounded-2xl animate-fade-in">
                        <label class="font-bold text-slate-700 block mb-2">Berikan Nilai (0-100)</label>
                        <div class="relative max-w-[200px]">
                            <input type="number" name="score" class="form-control text-2xl font-bold py-3 pl-4 pr-16" min="0" max="100" placeholder="0" required>
                    ...
                    <button type="submit" class="btn btn-primary w-full py-3.5 text-base shadow-lg shadow-indigo-500/20 hover:scale-[1.01] active:scale-[0.99] transition-all">
            ...
            <div class="card p-0 overflow-hidden border border-slate-200">
                @php
                    $colors = match($task->status) {
                        'completed' => ['bg' => 'bg-emerald-50', 'text' => 'text-emerald-700', 'icon' => 'text-emerald-600', 'icon_bg' => 'bg-emerald-100', 'border' => 'border-emerald-100'],
                        'submitted' => ['bg' => 'bg-sky-50', 'text' => 'text-sky-700', 'icon' => 'text-sky-600', 'icon_bg' => 'bg-sky-100', 'border' => 'border-sky-100'],
                        'revision' => ['bg' => 'bg-amber-50', 'text' => 'text-amber-700', 'icon' => 'text-amber-600', 'icon_bg' => 'bg-amber-100', 'border' => 'border-amber-100'],
                        default => ['bg' => 'bg-slate-50', 'text' => 'text-slate-700', 'icon' => 'text-slate-600', 'icon_bg' => 'bg-slate-100', 'border' => 'border-slate-100']
                    };
                @endphp

                <div class="{{ $colors['bg'] }} p-6 border-b {{ $colors['border'] }} flex items-center gap-4">
                    <div class="w-12 h-12 rounded-xl {{ $colors['icon_bg'] }} {{ $colors['icon'] }} flex items-center justify-center shrink-0">
                         @if($task->status === 'completed') <i class="fas fa-check-circle text-2xl"></i>
                         @elseif($task->status === 'revision') <i class="fas fa-history text-2xl"></i>
                         @else <i class="fas fa-hourglass-half text-2xl"></i>
                         @endif
                    </div>
                    <div>
                        <h3 class="font-bold text-lg {{ $colors['text'] }}">
                            @if($task->status === 'completed') Tugas Selesai
                            @elseif($task->status === 'revision') Riwayat Pengumpulan
                            @else Menunggu Review
                            @endif
                        </h3>
                         <div class="text-sm text-slate-500 flex items-center gap-2 mt-0.5">
                            <span><i class="far fa-clock mr-1"></i> Dikumpulkan {{ $task->submitted_at?->diffForHumans() ?? '-' }}</span>
                            @if($task->is_late)
                                <span class="badge badge-warning text-[10px] px-2 py-0.5 uppercase tracking-wide">Terlambat</span>
                            @else
                                <span class="badge badge-success text-[10px] px-2 py-0.5 uppercase tracking-wide">Tepat Waktu</span>
                            @endif
                        </div>
                    </div>
                </div>
                ...
                <div class="p-6 bg-white space-y-6">
                    ...
                    <div>
                        <span class="text-xs font-bold text-slate-400 uppercase tracking-wider block mb-2">Catatan Siswa</span>
